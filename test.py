...
def get_landscape(filename, a):
    ...
    for x in range(width):
        ...
        for y in range(height):
            landscape[x].append(pix[x, y] / a)
    return landscape


a = 50
h = get_landscape("map.bmp", a)
...
def draw(*args, **kwargs):
    ...
    for x in range(128):
        for y in range(128):
            if x%2 == 1 and y%2 == 1:
                glBegin(GL_TRIANGLE_FAN)
                glVertex3d(x / a, y / a, h[x][y])

                glVertex3d((x + 1) / a, y / a, h[x + 1][y])
                glVertex3d((x + 1) / a, (y + 1) / a, h[x + 1][y + 1])
                glVertex3d(x / a, (y + 1) / a, h[x][y + 1])
                glVertex3d((x - 1) / a, (y + 1) / a, h[x - 1][y + 1])
                glVertex3d((x - 1) / a, y / a, h[x - 1][y])
                glVertex3d((x - 1) / a, (y - 1) / a, h[x - 1][y - 1])
                glVertex3d(x / a, (y - 1) / a, h[x][y - 1])
